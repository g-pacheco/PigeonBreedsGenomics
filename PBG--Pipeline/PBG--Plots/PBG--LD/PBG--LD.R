setwd("/Users/hintze/Desktop/PhD\ Action\ Plan/Core\ Projects/Finishing/PBGP--FINAL/Analyses/PBGP--LD/")
...
opt$ld_files = "PBGP--GoodSamples_WithWGSs_NoOddSamplesNoFerals_NoCrupestris_SNPCalling_NoMissingData--Article--Ultra.LD.list"
opt$plot_x_lim = 507
opt$fit_level = 100
opt$plot_size = "2,5"
opt$max_kb_dist = 500
opt$fit_bin_size = 200
opt$col = 5
opt$bin_quant = 95
opt$fit_boot = 100
opt$plot_wrap_formula = "LD~File"
opt$plot_data = TRUE
opt$out_file = "PBGP--GoodSamples_WithWGSs_NoOddSamplesNoFerals_NoCrupestris_SNPCalling_NoMissingData--Article--Ultra.LD.pdf"
...
if(length(opt$ld) > 0) {
  ...
  if(opt$plot_data)
    ...
    scale_x_continuous("Genomic Distance (Kb)",
                     breaks = c(5e+04, 1e+05, 1.5e+05, 2e+05, 2.5e+05, 3e+05, 3.5e+05, 4e+05, 4.5e+05, 5e+05),
                     expand = c(0,0),
                     labels = c("50", "100", "150", "200", "250", "300", "350", "400", "450", "500"),
                     limits = c(0, 515000)) +
    ...
    theme(panel.grid.minor=element_blank(), panel.grid.major=element_blank(), plot.title=element_text(size=10)) +
    theme(axis.title.x = element_text(size=9, face="bold", color="#000000", margin = margin(t = 20, r = 0, b = 0, l = 0)),
          axis.title.y = element_text(size=9, face="bold", color="#000000", margin = margin(t = 0, r = 20, b = 0, l = 0))) +
    theme(axis.line = element_line(colour = "#000000", size = 0.3)) +
    theme(panel.border = element_blank()) +
    theme(axis.text = element_text(color="#000000", size=7)) +
    theme(axis.ticks = element_line(color="#000000", size=0.3)) +
...
### Save plot
ggsave(opt$out, plot=plot, height=3, width=5, scale=opt$plot_scale, dpi=1000, limitsize=FALSE)
x <- warnings()
